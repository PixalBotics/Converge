...
const CHAT_SCRIPT = `<script src=\"https://widget.company.com/chat-widget.js\" data-id=\"12345\" defer></script>`;
...
export default function ChatWidgetScriptPage() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(CHAT_SCRIPT);
    setCopied(true);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ border: `1px solid ${theme.app.dashboard.cardBorder}`, borderRadius: 1.5, p: 1.5, bgcolor: theme.app.dashboard.overlayLight }}>
        <Typography variant="body2" sx={{ color: theme.app.dashboard.textMuted, wordBreak: "break-all" }}>{CHAT_SCRIPT}</Typography>
      </Box>
    </WidgetFlowShell>
  );
}
