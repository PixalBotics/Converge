...
export function readVisitedModuleHrefs(userId: string | undefined | null): Set<string> {
  if (!userId?.trim() || typeof window === "undefined") return new Set();

  try {
    const raw = localStorage.getItem(dashboardModuleVisitsStorageKey(userId));
...
export function writeVisitedModuleHrefs(userId: string, hrefs: Iterable<string>): void {
  ...
  localStorage.setItem(dashboardModuleVisitsStorageKey(userId), JSON.stringify(next));
}
