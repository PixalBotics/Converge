...
/** Prefer server counts; fall back to list-derived counts when server omits zeros. */
export function reconcileBadgeCounts(
  ...
  const serverTotal =
    serverCounts.chat +
    serverCounts.qa +
    serverCounts.hrms_leave +
    serverCounts.hrms_attendance;
  const itemsTotal =
    fromItems.chat + fromItems.qa + fromItems.hrms_leave + fromItems.hrms_attendance;

  if (serverTotal >= itemsTotal) return serverCounts;
  return {
...
export function upsertConversationNotification(
  ...
  const cid = conversationId?.trim() || extractConversationId(incoming);
...
function extractConversationId(n: NotificationDto): string | null {
  const p = n.payload;
