...
/** Flat row for read-only POC UI (API contact + `user` or legacy invite fields). */
...
/**
 * Maps GET `/companies/parent/:id` POC data:
 * - `pocs[]` with nested `user` ({@link CompanyContactPoc})
 * - or legacy `pocInvite` flat fields ({@link CompanyPocInviteSummary})
 */
