...
export function loadDashboardAppearanceFromStorage(): DashboardAppearance {
  if (typeof window === "undefined") return defaultDashboardAppearance;
  return parseStoredDashboardAppearance(localStorage.getItem(DASHBOARD_APPEARANCE_STORAGE_KEY));
}

export function saveDashboardAppearanceToStorage(next: DashboardAppearance) {
  if (typeof window === "undefined") return;
  localStorage.setItem(DASHBOARD_APPEARANCE_STORAGE_KEY, JSON.stringify(next));
}
