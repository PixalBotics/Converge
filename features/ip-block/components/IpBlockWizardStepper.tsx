...
const STEPS = [
  { n: 1 as const, label: "Organization", hint: "Websites" },
  { n: 2 as const, label: "IP rule", hint: "Block address" },
];

export type IpBlockWizardStep = 1 | 2;

export function IpBlockWizardStepper({ currentStep }: { currentStep: IpBlockWizardStep }) {
    ...
    <Box sx={distributionStepperRootSx}>
      ...
      >
        <Typography variant="small" fontWeight={600} sx={{ color: theme.app.dashboard.textMuted }}>
          Step {currentStep} of {STEPS.length} · Blocked IPs cannot start or continue widget chat
        </Typography>
